...
/**
 * ============================================================================
 * PDF to PNG Tool — Phase 2A Wave 1
 * ----------------------------------------------------------------------------
 * Uses PDF.js to render PDF pages to canvas, then exports as PNG images.
...
export function PdfToPngTool({ tool, isBn, open, onOpenChange }: {
  tool: PdfTool
  isBn: boolean
  open: boolean
  ...
  const run = async () => {
    ...
    try {
      ...
      for (let i = 0; i < pagesToProcess.length; i++) {
        ...
        if (blob) {
          const name = `${baseName}-page-${pn}.png`
      ...
      if (images.length === 0) {
        ...
        toast.success(isBn ? `${bn(1)}টি PNG ডাউনলোড হয়েছে।` : `1 PNG downloaded.`)
      } else {
        const zip = new JSZip()
        ...
        saveAs(zipBlob, `${baseName}-png-images.zip`)
        toast.success(isBn ? `${bn(images.length)}টি PNG ZIP-এ ডাউনলোড হয়েছে।` : `${images.length} PNGs downloaded as ZIP.`)
      }
    } catch (err) {
      console.error('[PDF to PNG] Conversion failed:', err)
      toast.error(
        isBn
          ? 'PNG রূপান্তর ব্যর্থ — ফাইল নষ্ট বা এনক্রিপ্টেড হতে পারে।'
          : 'PNG conversion failed — file may be corrupt or encrypted.',
      )
    } finally {
      if (loadingTask) {
    ...
    <ToolDialog tool={tool} isBn={isBn} open={open} onOpenChange={onOpenChange}>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="space-y-1.5">
            <Label>{isBn ? 'রেজোলিউশন' : 'Resolution'}</Label>
            <Select value={quality} onValueChange={(v) => setQuality(v as QualityLevel)}>
              <SelectTrigger className="w-full"><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value="low">{isBn ? 'কম (১x)' : 'Low (1x)'}</SelectItem>
                <SelectItem value="medium">{isBn ? 'মাঝারি (১.৫x)' : 'Medium (1.5x)'}</SelectItem>
                <SelectItem value="high">{isBn ? 'উচ্চ (২x)' : 'High (2x)'}</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
        ...
        {extractedImages.length > 0 && !busy && (
          <div className="rounded-lg border border-border/60 bg-muted/30 p-4">
            <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
              {isBn ? `${bn(extractedImages.length)}টি PNG তৈরি হয়েছে` : `${extractedImages.length} PNGs generated`}
            </p>
        ...
        >
          {busy ? (
            ...
            <><ImageIcon className="mr-2 h-4 w-4" />{isBn ? 'PNG তৈরি করুন' : 'Convert to PNG'}</>
          )}
        </Button>
