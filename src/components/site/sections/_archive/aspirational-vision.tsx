...
export function AspirationalVision() {
  ...
  return (
    <section id="imagine" className="relative scroll-mt-24 overflow-hidden bg-gradient-to-br from-slate-950 via-emerald-950 to-slate-900 py-20 text-white sm:py-28">
      ...
      <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        <Reveal className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-5 text-lg text-emerald-100/70">{t('aspire.subtitle')}</p>
        </Reveal>
            ...
            return (
              <Reveal key={v.key} delay={i * 0.1}>
                <div className="flex items-start gap-4 rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur transition-colors hover:bg-white/10">
                  <div className="relative flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-600 to-cyan-500 text-white shadow-lg">
                    <span className="font-heading text-lg font-extrabold">{i + 1}</span>
                  </div>
                  <div className="flex-1 pt-1">
                    <p className="text-[15px] leading-relaxed text-emerald-50/90 sm:text-base">{t(v.key)}</p>
                  </div>
                </div>
              </Reveal>
            )
        ...
        <Reveal delay={0.3} className="mt-10 text-center">
          ...
          <button
            ...
            className="mt-6 inline-flex items-center gap-2 rounded-xl bg-white px-7 py-3.5 text-sm font-bold text-emerald-700 shadow-xl transition-transform hover:scale-[1.02]"
          >
            {t('aspire.makeItReal')}
